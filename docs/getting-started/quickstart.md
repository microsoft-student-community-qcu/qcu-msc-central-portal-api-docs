...
to `/api/v2` (see [Versioning](/docs/api/versioning)).

## 2. Try a public endpoint

The [OCR Verification API](/docs/api/v1/ocr) is fully public — it verifies a QCU
...
[Rate Limiting](/docs/intro#rate-limiting).
...
[Authentication](/docs/getting-started/authentication) before calling protected
endpoints.

## Next steps

- Browse the [API Reference](/docs/api/) for every v1 endpoint
- Follow a [workflow guide](/docs/guides/workflows) for multi-step flows like the applicant pipeline
- Check the [Setup Token Validation API](/docs/api/v1/setup-token) for device/setup authentication
